...
import { doc, getDoc, setDoc, onSnapshot, collection, addDoc, deleteDoc, getDocs } from "firebase/firestore";
...
const DEFAULT_LINKS = [
  { id: "canva", label: "Canva", url: "" },
  { id: "onenote", label: "OneNote", url: "" },
];

// Convertit l'ancien format { canva, onenote } en liste ouverte de liens,
// pour rester compatible avec les fiches projet créées avant ce changement.
function normalizeLinks(links) {
  if (Array.isArray(links)) return links;
  if (links && typeof links === "object") {
    return [
      { id: "canva", label: "Canva", url: links.canva ?? "" },
      { id: "onenote", label: "OneNote", url: links.onenote ?? "" },
    ];
  }
...
function getProjectDetail(ao, followedDate) {
  ...
  if (base) {
    return {
      ...base,
      links: normalizeLinks(base.links),
...
function Redaction({ aos, followed, projectData, ensureProject, updateProject }) {
  ...
  const movePiece = (section, pieceId, direction) =>
    updateProject(selected.id, (d) => {
      const list = [...d[section]];
      const idx = list.findIndex((pc) => pc.id === pieceId);
      const target = idx + direction;
      if (idx === -1 || target < 0 || target >= list.length) return d;
        ...
        return (
          ...
          >
            <div className="flex flex-col shrink-0">
              <button
                onClick={() => movePiece(section, piece.id, -1)}
                disabled={idx === 0}
                className="flex items-center justify-center"
                style={{ color: idx === 0 ? TOKENS.line : TOKENS.inkSoft, height: 12 }}
                aria-label="Monter"
              >
                <ChevronUp size={12} />
              </button>
              <button
                ...
                className="flex items-center justify-center"
                style={{ color: idx === list.length - 1 ? TOKENS.line : TOKENS.inkSoft, height: 12 }}
          ...
          <div className="flex flex-col gap-2 mb-8">
            {(detail.links ?? []).map((l) => (
              <div key={l.id} className="flex items-center gap-2">
                <input
                  value={l.label}
                  onChange={(e) => onUpdateLink(l.id, { label: e.target.value })}
                  placeholder="Nom du lien"
                  className="w-24 text-xs px-2 py-1.5 border outline-none shrink-0"
                  style={{ ...inputStyle, fontWeight: 500 }}
                />
                <input
                  value={l.url}
                  onChange={(e) => onUpdateLink(l.id, { url: e.target.value })}
                  placeholder={`Lien ${l.label || ""} du projet…`}
                ...
                <button onClick={() => onRemoveLink(l.id)} style={{ color: TOKENS.inkSoft }} aria-label="Supprimer ce lien">
                  <X size={13} />
                </button>
              </div>
            ))}
            {(detail.links ?? []).length === 0 && (
              <p className="text-[11px]" style={{ color: TOKENS.inkSoft, fontFamily: "'Inter', sans-serif" }}>Aucun lien pour l'instant.</p>
            )}
          </div>
...
/* ---------------------------------------------------------
   SAUVEGARDE (export .zip complet + restauration)
--------------------------------------------------------- */
...
function Sauvegarde({ aos, followed, statuts, followedDates, platforms, captureRef, activeTab, setActiveTab }) {
  const [exporting, setExporting] = useState(false);
  const [restoring, setRestoring] = useState(false);
  const [message, setMessage] = useState("");

  const buildFinanceSheet = (projects) => {
    const rows = [];
    Object.entries(projects).forEach(([aoId, d]) => {
      const ao = aos.find((a) => a.id === aoId);
      Object.entries(d.finance?.caByMonth ?? {}).forEach(([month, val]) => {
        const n = parseFloat((val ?? "").toString().replace(/[^\d.]/g, ""));
        if (!isNaN(n) && n !== 0) rows.push({ Mois: month, AO: aoId, Projet: ao?.titre ?? aoId, "CA (€)": n });
      });
    });
    return rows;
  };
  ...
  const handleExport = async () => {
    ...
    setMessage("");
    try {
      // 1. Récupère tous les projets réellement enregistrés (pas seulement ceux déjà chargés en mémoire)
      const projectsSnap = await getDocs(collection(db, "organisations", ORG_ID, "projects"));
      const projects = {};
      projectsSnap.forEach((d) => (projects[d.id] = d.data()));

      const data = {
        exportedAt: new Date().toISOString(),
        aos,
        projects,
        main: { followed, statuts, followedDates, platforms },
      };

      const zip = new JSZip();
      zip.file("sauvegarde.json", JSON.stringify(data, null, 2));

      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(aos), "AO");
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(buildFinanceSheet(projects)), "CA par projet");
      const wbArrayBuffer = XLSX.write(wb, { type: "array", bookType: "xlsx" });
      zip.file("sauvegarde.xlsx", wbArrayBuffer);

      if (captureRef?.current) {
        try {
          setActiveTab("ensemble");
          await new Promise((r) => setTimeout(r, 300));
          const canvas = await html2canvas(captureRef.current, { backgroundColor: "#EEF0EA" });
          const imgBlob = await new Promise((resolve) => canvas.toBlob(resolve, "image/png"));
          if (imgBlob) zip.file("capture-ecran-vue-ensemble.png", imgBlob);
          setActiveTab("sauvegarde");
        } catch (err) {
          console.error("Capture d'écran impossible :", err);
          setActiveTab("sauvegarde");
        }
      }
      ...
      downloadBlob(`sauvegarde-ao-paysage-${dateStr}.zip`, zipBlob);
      setMessage("Sauvegarde téléchargée.");
    } catch (err) {
      console.error(err);
      setMessage("Échec de la sauvegarde : " + err.message);
    } finally {
      setExporting(false);
    }
  };
  ...
  const handleRestore = async (e) => {
    ...
    if (
      !window.confirm(
        "Restaurer cette sauvegarde va écraser les AO, les fiches projet et les paramètres actuellement enregistrés. Continuer ?"
      )
    )
      return;
    ...
    try {
      ...
      if (!jsonFile) throw new Error("Fichier sauvegarde.json introuvable dans l'archive.");
      const data = JSON.parse(await jsonFile.async("string"));

      for (const ao of data.aos ?? []) {
        const { id, ...rest } = ao;
        await setDoc(doc(db, "organisations", ORG_ID, "aos", id), rest);
      }
      for (const [aoId, project] of Object.entries(data.projects ?? {})) {
        await setDoc(doc(db, "organisations", ORG_ID, "projects", aoId), project);
      }
      if (data.main) {
        await setDoc(doc(db, "organisations", ORG_ID, "app", "main"), data.main);
      }
      setMessage("Sauvegarde restaurée avec succès.");
    } catch (err) {
      console.error(err);
      setMessage("Échec de la restauration : " + err.message);
    } finally {
      setRestoring(false);
    }
  };
  ...
  return (
    <div className="p-6 max-w-xl">
      <h2 className="text-base mb-1" style={{ color: TOKENS.ink, fontFamily: "'Fraunces', serif", fontWeight: 600 }}>
        Sauvegarde de l'application
      </h2>
      <p className="text-xs mb-6" style={{ color: TOKENS.inkSoft, fontFamily: "'Inter', sans-serif" }}>
        Téléchargez une archive complète (AO, fiches projet, paramètres, un export Excel et une capture d'écran de
        la Vue d'ensemble) — utile pour garder une copie hors ligne ou pour restaurer les données en cas de besoin.
      </p>

      <div className="p-4 border mb-4" style={{ borderColor: TOKENS.line, background: "white" }}>
        <h3 className="text-xs uppercase tracking-wider mb-2 flex items-center gap-2" style={{ color: TOKENS.moss, fontFamily: "'JetBrains Mono', monospace" }}>
          <HardDriveDownload size={13} /> Télécharger une sauvegarde
        </h3>
        <button
        ...
        >
          ...
          {exporting ? "Préparation…" : "Télécharger la sauvegarde (.zip)"}
        </button>
      </div>

      <div className="p-4 border" style={{ borderColor: TOKENS.rust, background: TOKENS.rustDim }}>
        <h3 className="text-xs uppercase tracking-wider mb-2 flex items-center gap-2" style={{ color: TOKENS.rust, fontFamily: "'JetBrains Mono', monospace" }}>
          <RotateCcw size={13} /> Restaurer une sauvegarde
        </h3>
        <p className="text-[11px] mb-2" style={{ color: TOKENS.rust, fontFamily: "'Inter', sans-serif" }}>
          Attention : ceci remplace les données actuelles par celles de l'archive choisie.
        </p>
        <label
          className="flex items-center gap-2 text-sm px-3 py-2 cursor-pointer w-fit"
          style={{ background: "white", color: TOKENS.rust, fontFamily: "'Inter', sans-serif", fontWeight: 600, border: `1px solid ${TOKENS.rust}` }}
        >
          {restoring ? <Loader2 size={14} className="animate-spin" /> : <Upload size={14} />}
          {restoring ? "Restauration…" : "Choisir un fichier .zip"}
          <input type="file" accept=".zip" onChange={handleRestore} disabled={restoring} className="hidden" />
        </label>
      </div>

      {message && (
        <p className="text-xs mt-4" style={{ color: TOKENS.ink, fontFamily: "'Inter', sans-serif" }}>{message}</p>
      )}
    </div>
  );
}
    ...
    <div style={{ minHeight: "100%", background: TOKENS.paper }}>
      ...
      <div className="grid grid-cols-12">
        <div className="col-span-12 sm:col-span-2 border-r" style={{ borderColor: TOKENS.line }}>
          ...
          <NavItem icon={HardDriveDownload} label="Sauvegarde" active={activeTab === "sauvegarde"} onClick={() => setActiveTab("sauvegarde")} />
        ...
        <div className="col-span-12 sm:col-span-10" ref={captureRef}>
          {activeTab === "ensemble" && (
      ...
      {openProject && projectData[openProject.id] && (
        <ProjectDetail
          ...
          onAddLink={() =>
            updateProject(openProject.id, (d) => ({
              ...d,
              links: [...(d.links ?? []), { id: "link-" + Date.now(), label: "", url: "" }],
            }))
          }
          ...
          onRemoveLink={(linkId) =>
            updateProject(openProject.id, (d) => ({ ...d, links: (d.links ?? []).filter((l) => l.id !== linkId) }))
          }
          onAddMeeting={() => {
            const id = "r-" + Date.now();
            updateProject(openProject.id, (d) => ({
